...
const app = getApp<{
  globalData: {
    userInfo: wxNS.UserInfo;
  };
  userInfoReadyCallback(userInfo: wxNS.UserInfo): void;
}>()
...
Page({
  ...
  onLoad(options) {
    ...
    const app = getApp<{
      globalData: { userInfo: wxNS.UserInfo };
    }>()
  ...
  viewTap() {
    ...
    const p = getCurrentPages()[1] as wxNS.Page.Instance<
      { a: number },
      { customData: { b: number } }
    >
...
Page({
  ...
  jump() {
    ...
    query.select('#a').boundingClientRect(res => {
      expectType<wxNS.BoundingClientRectCallbackResult>(res)
    })
    query.selectViewport().scrollOffset(res => {
      expectType<wxNS.ScrollOffsetCallbackResult>(res)
    })
    query.exec(res => {
      expectType<any>(res)
    })
...
Page({
  test() {
    ...
    expectType<wxNS.EventChannel>(channel)
    channel.emit('test', {})
    channel.on('xxx', () => {})
...
Page({
  ...
  onShareAppMessage(): wxNS.Page.ICustomShareContent {
