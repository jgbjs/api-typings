    ...
    getContext() {
      wx.createSelectorQuery()
        ...
        .context(function(res) {
          const context = res.context as wxNS.VideoContext
          context.seek(0)
        })
        .exec()
    },
  ...
  Page({
    getNode() {
      wx.createSelectorQuery()
        .select('.canvas')
        .node(function(res) {
          const canvas = res.node as wxNS.Canvas
          canvas
        })
        .exec()
    },
...
// Test case from `VideoContext`
{
  ...
  Page({
    videoContext: {} as wxNS.VideoContext,
    onReady() {
...
// Test case from `wx.openSetting`
{
  wx.openSetting({
    success(res) {
      expectType<wxNS.AuthSetting>(res.authSetting)
      res.authSetting = {
        'scope.userInfo': true,
...
// Test case from `EditorContext.insertImage`
{
  wx.createSelectorQuery()
    .select('#editor')
    .context(res => {
      const editorCtx = res.context as wxNS.EditorContext
